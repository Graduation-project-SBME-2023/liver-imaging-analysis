...
from utils import calculate_largest_tumor,find_pix_dim,get_colors
import numpy as np
...
class Lesions_Visualization:
    def __init__(self,volume_nfti,liver_lesions_mask,idx=None,mode='contour',plot=True):
        self.volume_nfti=volume_nfti
        self.volume=volume_nfti.get_fdata()
        self.mask =torch.from_numpy(np.where(liver_lesions_mask == 2, 1, 0))
        self.idx=idx
        self.mode=mode
        self.plot=plot

    def visualize_tumor(self):
        """
        ...
        """
        self.volume = ScaleIntensityRange(
            a_min=-135,
            a_max=215,
            b_min=0.0,
            b_max=1.0,
        ...
        if self.mode == "contour":
            ...
            if self.idx is not None:
                ...
                calculations=self.major_axis_recursive(self.volume[:, :, self.idx], self.mask[:, :, self.idx], mode="slice")

            else:
                calculations=self.major_axis_recursive(self.volume, self.mask,mode="volume")

        if self.mode == "box":
            if self.idx is not None:

                ax.imshow(self.volume[:, :, self.idx], interpolation=None, cmap="gray")

                self.plot_bbox_image(self.mask[:, :, self.idx], crop_margin=0)
            else:
                self.plot_bbox_image_volume(self.volume, self.mask, crop_margin=0)
        ...
        if self.mode == "zoom":
            if self.idx is not None:
                ...
                self.plot_tumor(
                    self.volume[:, :, self.idx], self.mask[:, :, self.idx]
                )  

            else:
                self.plot_tumor_volume(self.volume, self.mask)
    ...
    def major_axis_recursive(self,volume, mask, mode="volume"):
        """
    ...
    #    This process is repeated until all tumors have been processed.
        ...
        calculations=[]
        x,y,z = find_pix_dim(self.volume_nfti) 
        ...
        while np.unique(mask).any() == 1:
            ...
            if mode == "volume":
                idx = calculate_largest_tumor(largest_tumor[0],self.volume_nfti)
                major,minor=self.major_axes(volume[:, :, idx], largest_tumor[0][:, :, idx], ax)
                ...
                major,minor=self.major_axes(volume, largest_tumor[0], ax)
    ...
    def major_axes(self,volume_slice, mask_slice, ax):
        """
    ...
    def get_bounding_box(self,mask_slice, crop_margin=0):
        """
    ...
    def plot_bbox_image(self,mask_slice, crop_margin=0, j=-1):
        """
        ...
        while np.unique(mask_slice).any() == 1:
            ...
            xmin, ymin, xmax, ymax = self.get_bounding_box(largest_tumor[0], crop_margin)
        ...
        x_dim, y_dim, z_dim = find_pix_dim(self.volume_nfti)   # again same calculation
    ...
    def plot_bbox_image_volume(self,volume, mask, crop_margin=0):
        """
        ...
        while np.unique(mask).any() == 1:
            ...
            self.plot_bbox_image(largest_tumor[0][:, :, idx], crop_margin, i)
            i = i + 1

            mask = subtract(mask, largest_tumor[0])


    def crop_to_bbox(self,image, bbox, crop_margin=0, pad=40):
        """
    ...
    def plot_tumor(self,volume_slice, mask_slice):
        """
        ...
        xmin, ymin, xmax, ymax = self.get_bounding_box(mask, crop_margin=0)
        ...
        croped_image = self.crop_to_bbox(image, bbox, crop_margin=0)
        croped_image = cv.resize(
            croped_image, dsize=(512, 512), interpolation=cv.INTER_CUBIC
        )
        croped_masks = self.crop_to_bbox(mask, bbox, crop_margin=0)

        croped_masks = cv.resize(
            croped_masks, dsize=(512, 512), interpolation=cv.INTER_NEAREST
        )
        ...
        self.major_axes(croped_image, croped_masks, ax[2])
    ...
    def plot_tumor_volume(self,volume, mask):
        """
        ...
        while np.unique(mask).any() == 1:
            ...
            largest_tumor = KeepLargestConnectedComponent()(temp_mask)
            idx = calculate_largest_tumor(largest_tumor[0],self.volume_nfti)
            self.plot_tumor(volume[:, :, idx], largest_tumor[0][:, :, idx])
            mask = subtract(mask, largest_tumor[0])
            first_tumor = False
