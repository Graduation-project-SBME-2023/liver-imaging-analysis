...
def calculate_largest_tumor(mask,volume_nfti):
    """
    ...
    max_volume = -1
    idx = -1
    x, y, z = find_pix_dim(volume_nfti)
    clone = mask.clone()
...
def find_pix_dim(volume):
    ...
    ----------
    volume: nfti object
        the nfti object of the patient volume.
    Returns
    -------
    list
...
def transform_to_hu(nfti_img):
    """
    a function to transform the input nfti to its Hounsfield values
    Parameters
    ----------
    nfti_img: NFTI object
        the NFTI object of the volume to be transformed
    Return
    ----------
    HU_data: numpy array
    ...
    img_data = nfti_img.get_fdata()

    # Get the slope and intercept values for the Hounsfield unit (HU) calculation
    slope = nfti_img.dataobj.slope
    intercept = nfti_img.dataobj.inter
...
def stringify_dictionary(dictionary, indent=""):
    ...
    Recursively converts a dictionary into a string representation.
        ...
        indent : str,optional 
            The indentation string used for formatting. Defaults to "".
