...
def test_predict_2dto3d(lesion_obj):
    ...
    lesion_inference = "3D"
    volume_dir = "tests/testdata/resized_liver/volume/resized_liver.nii"
...
def test_train():
    ...
    train_lesion(
        pretrained=False,
        cp_path="tests/testdata/lesion_cp_tr",
        epochs=1,
        evaluate_epochs=1,
        batch_callback_epochs=100,
        save_weight=True,
        save_path="tests/testdata/lesion_cp_tr",
        test_batch_callback=False,
    )
    model.load_checkpoint("tests/testdata/lesion_cp_tr")
