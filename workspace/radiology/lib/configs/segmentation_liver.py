...
class SegmentationLiver(TaskConfig):
    def __init__(self):
        super().__init__()
    ...
    def init(self, name: str, model_dir: str, conf: Dict[str, str], planner: Any, **kwargs):
        ...
        # Model Files
        self.path = [
            os.path.join(self.model_dir, f"pretrained_liver.pt"),  # pretrained
            os.path.join(self.model_dir, f"pretrained_liver.pt"),  # published
        ]
    ...
    def infer(self) -> Union[InferTask, Dict[str, InferTask]]:
        task: InferTask = lib.infers.SegmentationLiver(
            path=self.path,
    ...
    def trainer(self) -> Optional[TrainTask]:
        ...
        task: TrainTask = lib.trainers.SegmentationLiver(
            model_dir=output_dir,
            network=self.network,
            description="Train Liver Segmentation Model",
            load_path=load_path,
    ...
    def scoring_method(self) -> Union[None, ScoringMethod, Dict[str, ScoringMethod]]:
        ...
        if self.epistemic_enabled:
            methods[f"{self.name}_epistemic"] = EpistemicScoring(
                ...
                transforms=lib.infers.SegmentationLiver(None).pre_transforms(),
